...
var c=/^[A-Za-z0-9]+$/;
	if (!c.test(password)) {
		$(currentForm).find('#password_notice').html("登录密码不能包含空格等特殊字符");
...
function checkMobilePhoneExist(mobile, callback,token) {
	var mobileObj = null;
	if (typeof (mobile) == 'object') {
		mobileObj = $(mobile);
		mobile = mobileObj.val();
 ...
 */
function sendMobileCode(mobileObj, mobileCodeObj, sendButton,token) {
	checkMobilePhone(mobileObj, function(result) {

		if (result) {
			// 发送邮件
			var url = '/regist/getcode';
